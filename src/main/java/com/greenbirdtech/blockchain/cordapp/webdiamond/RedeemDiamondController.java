...
@Controller
public class RedeemDiamondController 
{
	private final static int REDEEMDIAMOND=1;
	
	private final static Logger logger = LogManager.getLogger(RedeemDiamondController.class);
	
	@Autowired
	private MenuInf menuinf;
	
	@RequestMapping(value = "/redeemdiamond", method = {RequestMethod.POST,RequestMethod.GET})
	public ModelAndView redeemdiamond(final HttpServletRequest request,final HttpServletResponse response) throws Exception
	{
		...
		if (menuinf.checkUserPermission(request))
		{
			...
			String retStr=null;
			ModelAndView mav = new ModelAndView("redeemdiamond");
						...
						if ((strRet.length == 2) && !StringUtil.isNull(strRet[0]) && !StringUtil.isNull(strRet[1]))
						{
							try
							{
								...
								if ((strUUID.length == 2) && !StringUtil.isNull(strUUID[0]) && !StringUtil.isNull(strUUID[1]))
								{
									...
									retStr = diamondtradeinf.redeemDiamond(linearid);
								}
							}
							catch (NumberFormatException nfe)
							{
							...
							{
								logger.warn("Failure to redeem diamond:"+de.toString());
								retStr = de.getMsg();
							}
						}
					}
