...
@Controller
public class AuditDiamondController 
{
	private final static int AUDITDIAMOND=1;
	private final static int AUDITRESPDIAMOND=2;
	
	private final static Logger logger = LogManager.getLogger(AuditDiamondController.class);
	
	@Autowired
	private MenuInf menuinf;
	
	@RequestMapping(value = "/auditdiamond", method = {RequestMethod.POST, RequestMethod.GET})
	public ModelAndView transferdiamond(final HttpServletRequest request,final HttpServletResponse response) throws Exception
	{
		...
		if (menuinf.checkUserPermission(request))
		{
			...
			ModelAndView mav = new ModelAndView("auditdiamond");
			...
			if (diamondtradeinf != null)
			{
				...
				if ((reqcode == AUDITDIAMOND) || (reqcode == AUDITRESPDIAMOND))
				{
						...
						if ((strRet.length == 3) && !StringUtil.isNull(strRet[0]) && !StringUtil.isNull(strRet[1]) && !StringUtil.isNull(strRet[2]))
						{
							try
							{
								...
								if ((strUUID.length == 2) && !StringUtil.isNull(strUUID[0]) && !StringUtil.isNull(strUUID[1]))
								{
									...
									if (reqcode == AUDITDIAMOND)
										retStr = diamondtradeinf.auditDiamond(linearid,strRet[2]);
									else
										retStr = diamondtradeinf.auditrespDiamond(linearid,strRet[2]);
								}
							}
							catch (NumberFormatException nfe)
							{
							...
							{
								logger.warn("Failure to audit diamond:"+de.toString());
								retStr = de.getMsg();
							}
						}
					}
